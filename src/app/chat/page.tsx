...
export default function ChatPage() {
  ...
  const handleSend = async (overrideMessage?: string) => {
    ...
    try {
      ...
      if (res.status === 503 && data.fallback) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            role: 'ai',
            text: '⚠️ AI model not connected. Add your `GEMINI_API_KEY` to `.env.local` to enable real AI responses.',
            timestamp: new Date().toISOString(),
          },
        ]);
        return;
      ...
      const isAlreadyInSidebar = conversations.some(c => c.id === activeConvId);
  ...
  return (
    <div className="flex h-screen bg-[#0B0F19] overflow-hidden text-porcelain font-sans p-0 sm:p-4 lg:p-8 gap-0 sm:gap-4 lg:gap-6 relative">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-72 sm:w-80 bg-[#161C24] sm:rounded-2xl flex flex-col overflow-hidden shadow-2xl transition-transform duration-300 ease-out lg:translate-x-0 lg:relative lg:flex lg:z-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} print:hidden`}>
        ...
        <div className="p-5">
          <button onClick={startNewConversation} className="flex items-center space-x-3 w-full px-5 py-3.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-full transition-colors font-medium text-sm text-porcelain shadow-sm hover:shadow-[0_0_15px_rgba(45,212,191,0.2)] focus:outline-none">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-1">
          ...
          {conversations.length === 0 ? (
            ...
            conversations.map((conv) => (
              <button key={conv.id} onClick={() => loadConversation(conv.id)} className={`w-full text-left px-4 py-3 rounded-xl transition-colors text-sm flex items-center group relative ${activeConvId === conv.id ? 'bg-teal-500/10 text-teal-400' : 'text-slate-400 hover:text-porcelain hover:bg-white/[0.04]'}`}>
                <svg className="w-4 h-4 mr-3 flex-shrink-0 text-slate-500 group-hover:text-teal-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
                <span className="truncate flex-1">{conv.title}</span>
                <span role="button" onClick={(e) => handleDeleteConv(e, conv.id)} className="ml-2 opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-400 transition-all flex-shrink-0 cursor-pointer" title="Delete">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-full">
              ...
              <div className="min-h-full flex flex-col">
                <div className="m-auto flex flex-col items-center w-full max-w-3xl py-8 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
                  ...
                  <h1 className="text-3xl sm:text-4xl lg:text-5xl font-semibold tracking-tight text-center text-transparent bg-clip-text bg-gradient-to-r from-slate-200 to-teal-300 max-w-2xl leading-tight mb-2">
                    Hello, {user?.role || 'Manager'}.<br />What shall we optimize today?
                  </h1>
                  {apiWarning && (
                    <div className="w-full max-w-lg bg-amber-500/10 border border-amber-500/30 rounded-xl px-5 py-3 text-xs text-amber-400 text-center">
                      AI model not configured. Check your <code className="font-mono bg-black/30 px-1 rounded">GEMINI_API_KEY</code>.
                    </div>
                  )}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6 w-full">
                    {SUGGESTION_CARDS.map((card) => (
                      <button key={card.label} onClick={() => handleSend(card.prompt)} disabled={isThinking} className="flex text-left items-start p-6 rounded-3xl bg-white/[0.02] border border-transparent hover:bg-white/[0.04] hover:border-teal-500/20 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(45,212,191,0.1)] transition-all duration-300 group cursor-pointer disabled:opacity-50">
                        <div className="bg-white/5 p-2 rounded-xl mr-4 group-hover:bg-teal-500/10 transition-colors">
                          <svg className="w-5 h-5 text-slate-400 group-hover:text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={card.icon} /></svg>
                        </div>
              ...
              <div className="w-full max-w-3xl mx-auto space-y-8 pb-4">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                    ...
                    <div className={`max-w-[85%] rounded-3xl px-6 py-4 text-[15px] ${msg.role === 'user' ? 'bg-slate-800 text-porcelain' : 'text-slate-200'}`}>
                      {msg.role === 'ai' ? (
                        <div className="prose prose-invert prose-sm max-w-none [&_table]:w-full [&_table]:border-collapse [&_table]:my-4 [&_table]:border [&_table]:border-white/20 [&_th]:border [&_th]:border-white/20 [&_th]:p-3 [&_th]:bg-white/10 [&_th]:text-left [&_td]:border [&_td]:border-white/10 [&_td]:p-3">
        ...
        <div className="shrink-0 pb-4 sm:pb-6 px-2 w-full max-w-3xl mx-auto">
          <div className="relative group shadow-[0_10px_40px_rgba(0,0,0,0.4)] rounded-full border border-white/[0.04]">
            <input ref={inputRef} type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) handleSend(); }} placeholder="Query the logistics matrix..." disabled={isThinking} className="w-full bg-[#1A222C] border border-white/[0.05] rounded-full pl-6 pr-32 py-4 sm:py-5 text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-teal-500/50 focus:ring-1 focus:ring-teal-500/50 focus:bg-[#1E2733] transition-all text-[15px] disabled:opacity-60" />
            <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center space-x-1">
              <button onClick={() => handleSend()} disabled={!inputMessage.trim() || isThinking} className={`p-2.5 rounded-full transition-all duration-300 flex items-center justify-center ${inputMessage.trim() && !isThinking ? 'bg-teal-500 text-slate-900 shadow-[0_0_15px_rgba(45,212,191,0.5)] hover:bg-teal-400 hover:scale-105' : 'bg-transparent text-slate-500 pointer-events-none'}`} aria-label="Send Message">
                {isThinking ? (
                  <div className="w-5 h-5 border-2 border-slate-900 border-t-transparent rounded-full animate-spin" />
                ) : (
                  <svg className="w-5 h-5 rotate-90 ml-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
                )}
              </button>
            </div>
          </div>

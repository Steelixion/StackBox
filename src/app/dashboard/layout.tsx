...
import { getUserProfile } from '@/actions/auth'; // Import your Action
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // ── Load session via Server Action ───────────────────────────────────────
  ...
  const navItems = [
    ...
    { name: 'Employees', href: '/dashboard/employees', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
  ];
            ...
            <div ref={profileRef} className="relative">
              <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center space-x-3 p-1 hover:bg-porcelain rounded-xl transition-colors">
                {user?.picture ? (
                  ...
                  <div className={`w-8 h-8 ${loading ? 'bg-steel/10 animate-pulse' : 'bg-deep-ink text-white'} rounded-full flex items-center justify-center text-xs font-bold`}>
                ...
                <div className="hidden sm:block text-left">
                  {loading ? (
                    ...
                    <>
                      ...
                      <p className="text-[10px] text-steel font-bold uppercase mt-1 tracking-wider">{user?.role}</p>
                    </>
                  )}
                </div>
              </button>
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-steel/10 rounded-xl shadow-xl z-50 p-1">
                  <button onClick={handleLogout} className="w-full text-left p-3 text-sm text-cinnabar hover:bg-cinnabar/5 rounded-lg flex items-center">
                    Sign Out
                  </button>
                </div>
              )}
